...
param([Parameter(Mandatory=$true)][ipaddress]$Address,[ValidateRange(1,65535)][int]$Port=47831)
...
New-NetFirewallRule -Name $ruleName -DisplayName "Codex Pet wireless sync ($Port)" -Direction Inbound -Action Allow -Protocol TCP -LocalPort $Port -LocalAddress $Address.ToString() -RemoteAddress LocalSubnet -InterfaceAlias $local.InterfaceAlias -Profile Any -ErrorAction Stop | Out-Null
Write-Host 'Wireless sync allowed only from the local subnet on the selected interface.'
