...
默认关闭同步；可配置随码团启动、端口和配对码有效期。只分享五小时／一周额度、重置时间和同步时间，电脑登录信息留在电脑上。v2.2.2 限制单个来源的握手并发和请求大小，修复限流状态可被持续撑大的问题；Android 配对只接受私有局域网或明确的 USB 本机地址，并严格限制所有配对字段。详细连接和排查说明见 [手机连接说明](mobile-sync/README.md)。手机端请安装 [Android 0.2.2](https://github.com/Rxwkovo/Codex-Usage-Pet/releases/tag/android-v0.2.2)。
...
- **右下角系统托盘**：右键可显示码团、打开设置、连接手机、手动刷新、暂停／恢复动作、切换置顶、查看 Codex 用量页或退出。双击图标展开码团；图标可能位于托盘折叠菜单。
...
v2.2.2 解压到 `%LOCALAPPDATA%\CodexUsagePet\2.2.2`，首次运行优先迁移 v2.2.1 的位置、大小、频率等设置，并按版本顺序回退查找旧设置。重复启动 EXE 不会重复创建宠物。退出后删除 EXE 和对应版本目录即可卸载；没有开机自启。
