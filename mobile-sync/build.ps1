...
& $Python -m PyInstaller --noconfirm --clean --onedir --windowed --name CodexPet-Mobile-Link --distpath (Join-Path $PSScriptRoot 'dist') --workpath (Join-Path $PSScriptRoot 'build') --specpath (Join-Path $PSScriptRoot 'build') --copy-metadata qrcode --copy-metadata Pillow --copy-metadata PyInstaller --exclude-module numpy --exclude-module matplotlib --exclude-module tkinter --exclude-module pytest --icon (Join-Path $PSScriptRoot '../pet.ico') (Join-Path $PSScriptRoot 'desktop.py')
