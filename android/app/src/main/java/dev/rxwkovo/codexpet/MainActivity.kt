...
class MainActivity:Activity() {
    ...
    private fun render(){
        ...
        fun label(title:String,w:WindowQuota?)="$title   ${w?.remaining?.let{if(it.isFinite()&&it in 0.0..100.0)"${it.toInt()}%" else "—"}?:"—"}"+
            if(w!=null && w.resetsAt>0)"\n"+"恢复于 "+SimpleDateFormat("MM-dd HH:mm",Locale.getDefault()).format(Date(w.resetsAt*1000)) else ""
    ...
    @Deprecated("Legacy result bridge") override fun onActivityResult(r:Int,result:Int,data:Intent?){
        ...
        try{
            ...
            val bitmap=contentResolver.openInputStream(uri).use{BitmapFactory.decodeStream(it,null,options)}?:error("image")
            val pixels=IntArray(bitmap.width*bitmap.height);bitmap.getPixels(pixels,0,bitmap.width,0,0,bitmap.width,bitmap.height)
            val code=MultiFormatReader().decode(BinaryBitmap(HybridBinarizer(RGBLuminanceSource(bitmap.width,bitmap.height,pixels)))).text
            bitmap.recycle();pair(code)
        }catch(e:Exception){toast("没有找到配对二维码，请使用原图或粘贴配对码")}
    }
