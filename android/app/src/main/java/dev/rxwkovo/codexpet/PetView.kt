...
class PetView @JvmOverloads constructor(context:Context, val floating:Boolean=false):View(context) {
    ...
    private var lastGood:Bitmap?=null
    private var prefsAt=0L
    private var prefRandom=true;private var prefCompact=true;private var prefStretchHold=18
    private val typeface=Typeface.create("sans-serif-medium",Typeface.NORMAL)
    // SharedPreferences reads take a lock each call; the draw loop did three of them
    // per frame. Refresh at most once a second instead, which is still instant to a user.
    private fun refreshPrefs(now:Long){
        if(now-prefsAt<1000)return
        prefsAt=now
        prefRandom=store.prefs.getBoolean("random",true)
        prefCompact=store.prefs.getBoolean("compact",true)
        prefStretchHold=store.prefs.getInt("stretchHold",18)
    }
    // Decoding a 440x420 PNG costs a few milliseconds, which is a visible stutter when it
    // happens inside onDraw. Decode on the shared worker and keep the frame already on
    // screen until the new one lands. Only the very first frame blocks, so the pet is not
    // invisible at start-up; everything after that is off the draw thread.
    ...
    private fun decode(name:String):Bitmap?=
        try{context.assets.open("pet/$name.png").use{BitmapFactory.decodeStream(it)}}catch(e:Exception){null}
    private fun frame(name:String):Bitmap?{
        frames.get(name)?.let{lastGood=it;return it}
        if(lastGood==null){
            decode(name)?.let{frames.put(name,it);lastGood=it;return it}
        }
        if(pending.add(name)){
            loader.execute{
                ...
                main.post{
                    pending.remove(name)
                    if(bitmap!=null){frames.put(name,bitmap);invalidate()}
                }
            }
        }
        return lastGood
    ...
    private fun update(){
        ...
        if(prefRandom && now>=nextAction && motion.action in listOf("idle","compact"))action(listOf("walk","sit","sleep","wave","stretch").random())
        if(floating && prefCompact && now-lastTouch>20000 && motion.action=="idle")motion.start("compact",now)
        // The switch only ever controlled entering compact. compact lasts an hour
        // (Domain.kt), so turning it off left an already-curled pet curled until a
        // random action happened to interrupt - and with random actions off it looked
        // like the switch did nothing at all.
        else if(floating && !prefCompact && motion.action=="compact")motion.start("idle",now)
        if(now>blinkAt+160)blinkAt=now+Random.nextLong(2500,6000)
    }
    override fun onDraw(canvas:Canvas){
        super.onDraw(canvas)
        val now=SystemClock.uptimeMillis()
        motion.stretchHoldPercent=prefStretchHold
        val mood=store.mood()
        ...
        if(floating && !compact){
            ...
            paint.typeface=typeface;paint.textSize=11*resources.displayMetrics.density
    ...
    override fun onTouchEvent(e:MotionEvent):Boolean {
        when(e.actionMasked){
            ...
            MotionEvent.ACTION_UP->{if(dragged)dragEnd?.invoke() else if(SystemClock.uptimeMillis()-downAt>600)openApp?.invoke() else performClick();return true}
            MotionEvent.ACTION_CANCEL->{if(dragged)dragEnd?.invoke();return true}
        };return true
    }
    ...
    companion object{
        // One decode thread for the whole process: both pet views share it, and it is a
        // daemon so it can never hold the process open.
        private val loader=java.util.concurrent.Executors.newSingleThreadExecutor{r->Thread(r,"pet-frame-loader").apply{isDaemon=true}}
