...
internal static class Launcher
{
    ...
    private static int Main(string[] args)
    {
        ...
        using (var mutex = new Mutex(true, "Local\\CodexUsagePet" + (smoke ? "Smoke" : ""), out first))
        {
            ...
            try
            {
                string root = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "CodexUsagePet", "2.2.2");
                ...
                if (!smoke)
                {
                    ...
                    {
                        string oldFile = "";
                        foreach (string version in new[] { "2.2.1", "2.2.0", "2.1.2", "2.1.1", "2.1.0", "2.0.0", "1.3.1" })
                        {
